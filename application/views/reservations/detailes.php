...
<section class="reservation-checked-in-list">
	<div class="row">
		<div class="col-md-12">
		<button class="btn btn-sm float-right mr-2 btn-danger mb-3" onclick="printDiv()">Print Details</button>
		<?php if($row->status !=4){?>
		<a href="<?=base_url('reservations-doc/'.$row->id);?>/yes" class="btn btn-primary btn-sm js-click no-alert checkout-open mb-1 mr-2 float-right" data-target="#checkout" >Document</a>
		<a href="<?=$check_out_url?><?=$row->id?>" class="btn btn-primary btn-sm js-click no-alert checkout-open mr-1 mb-1 float-right " data-target="#checkout" >Check Out</a>
		<a href="<?=base_url('checkin/'.$row->id);?>" class="btn btn-primary btn-sm js-click no-alert checkout-open mr-1 mb-1 float-right " data-target="#checkout" >Checkin</a>
	  <?php }?>
			...
			<table class="table table-striped table-bordered">
				...
				<tfoot>
					<tr>
						...
						<td int> <?=setting()->currency;?> <?= _number_format((((((($row->tax_value+$row->total_withouttax)+$extra_amount)-$tr->credit)-$amount['pre_checkin_amount'])-$trCheckoutAmt)-$trTransactionAmt)+$recieved)?></td>
