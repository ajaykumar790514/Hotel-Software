...
<script type="text/javascript">
	$('.activities .activity').change(function(event){
	   ...
	   $.post('<?=base_url()?>properties/activity/<?=$property_id?>',{a_id:id,type:type})
		.done(function(data){
			console.log(data);
	...
	$(document).on('keyup change','.activities .activity_data' ,function(event) {
		...
		timer = setTimeout(function() {
			...
			var type = 'data';  
			$.post('<?=base_url()?>properties/activity/<?=$property_id?>',{a_id:id,type:type,cloumn:cloumn,value:value})
			.done(function(data){
				console.log(data);
				data = JSON.parse(data);
				if (data.res=="error") {
