                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <?php if($isValid==0){
                                     echo '<h4 id="expirationMsg" class="text-danger">Sorry, your plan has been expired. Please upgrade your plan.</h4>';
                                     echo '<script>
                                             setTimeout(function() {
                                ...
                                {
                                    echo '<h4 id="expirationMsg2" class="text-danger">Sorry, Please select plan first.</h4>';
                                    echo '<script>
                                            setTimeout(function() {
                                                document.getElementById("expirationMsg2").innerHTML = "";
                                            }, 5000); // Hide the message after 5 seconds (adjust as needed)
                                          </script>';   
                                };?>
                            ...
                            <div class="container-fluid ml-5">
                                ...
                                <?php foreach($user_packages_master as $packages):
                                    if($packages->id==$plan->package_id){?>
                                      <div class="card gradient-card col-md-3 col-sm-3 col-lg-3 ml-5">
                                    <div class="card-body gradient-card-body">
                                        <h2 class="card-title text-white text-center" style="font-size: 2rem;font-weight:bold"><?=@$plan->name?>
                                        <i class="la la-check-circle ml-1 " style="color:white;font-size:1.2rem">Current Plan</i></h2>
                                        <p class="text-center"><img src="<?=IMGS_URL.@$plan->pic;?>" alt="" height="70px"></p>
                                        <h5 class="card-text text-white text-center">Valitidy : <?=@$plan->no_of_days;?> Days</h5>
                                        <h3 class="card-text text-white text-center">Price : <?=setting()->currency;?><?=@$plan->price;?></h3>
                                        <p class="card-text text-white text-center"><?=@$plan->description;?> Days</p>
                                    </div>
                                    <div class="card-body gradient-card-footer">
                                    <center><button class="btn btn-success"><b>Current Plan</b></button></center>
                                    </div>
                                </div>
                               <?php }else{?>
                                <div class="card gradient-card col-md-3 col-sm-3 col-lg-3 ml-5">
                                    ...
                                    <div class="card-body gradient-card-footer">
                                    <center><button onclick="upgradeplan(<?=$packages->id;?>,<?=$packages->max_room;?>)" id="upgradeplan_<?=$packages->id;?>" class="btn btn-success"><b>Upgrade Plan </b></button></center>
                                    </div>
...
<script>
 ...
 function upgradeplan(plan,max_room){
    ...
    $.ajax({
    url: '<?=base_url('upgradeplan/'.$row->id)?>',
    type:'POST',
    ...
    {
     btn.attr('disabled', false).text('Please Wait').css('padding','11px 16px');
     if (res.flag === 'package') {
