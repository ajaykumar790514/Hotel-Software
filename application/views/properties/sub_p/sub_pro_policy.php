...
<script type="text/javascript">
	$('.policies .apolicy').change(function(event){
	   ...
	   $.post('<?=base_url()?>sub_properties/<?=$pro_id?>/policy/<?=$flat_id?>',{p_id:id,type:type})
		.done(function(data){
			// console.log(data);
	...
	$(document).on('keyup change','.policies .policy_data' ,function(event) {
		...
		timer = setTimeout(function() {
			...
			var type = 'data';  
			$.post('<?=base_url()?>sub_properties/<?=$pro_id?>/policy/<?=$flat_id?>',{p_id:id,type:type,cloumn:cloumn,value:value})
			.done(function(data){
				console.log(data);
				data = JSON.parse(data);
				if (data.res=="error") {
