		...
		<table class="table table-striped table-bordered base-styl amenities">
			...
			<?php foreach ($amenities as $row) { ?>
				<tr>
					...
					<td><?=$row->name?></td>
					<td><img src="<?=base_url('assets/icons/'.$row->icon)?>" class="icon-sm"></td>
					<td>
...
<script type="text/javascript">
	$('.amenities .amenity').change(function(event){
	   ...
	   $.post('<?=base_url()?>sub_properties/<?=$pro_id?>/amenities/<?=$flat_id?>',{a_id:id,type:type})
		.done(function(data){
			console.log(data);
	...
	$(document).on('keyup change','.amenities .amenity_data' ,function(event) {
		...
		timer = setTimeout(function() {
			...
			var type = 'data';  
			$.post('<?=base_url()?>sub_properties/<?=$pro_id?>/amenities/<?=$flat_id?>',{a_id:id,type:type,cloumn:cloumn,value:value})
			.done(function(data){
				console.log(data);
				data = JSON.parse(data);
				if (data.res=="error") {
